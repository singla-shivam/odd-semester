function DES(message) {
    ...
    const expansionBox = [32, 1, 2, 3, 4, 5,
        4,  5,  6 , 7  ,8 , 9,
        8,  9, 10, 11, 12, 13,
       12, 13, 14, 15, 16, 17,
       16, 17, 18, 19, 20, 21,
    ...
    for(let i = 0, len = binaryMessage.length; i < len; i += 64) {
        ...
        let left = blockAfterIP.slice(0, 32)
        let right = blockAfterIP.slice(32)
        for(let j = 0; j < 16; j ++) {
            const r = xor(functionF(roundKeys[i], right), left)
            if(j < 15){
                left = right
                right = r
            }
    ...
    function functionF(key, rightI) {
        // TODO
        const expanded = permute(expansionBox, rightI)
